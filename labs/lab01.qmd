...
I recommend the following settings: (If you encounter "Error: Permission Denied", see [this](https://forum.posit.co/t/need-help-with-global-options-in-rstudio/118630/2) for help.)
...
can be manually triggered by running `xcode-select --install` at a command line.[^commandline]

[^commandline]: Note that this must be run at a 
**Mac command line**, not at the `R` console. The *command line*
is direct control of the operating system itself, while the
`R` console is just for controlling `R`. Within `RStudio`, you can
access the command line by clicking on the `Terminal` tab, located
