---
lab_title: "`R`ev your Engines! Setting-Up `R` and `RStudio`"
lab_num: "`r lab <- 1; lab`"
---
...
library(tidyverse)
library(rvest)
latest_R <- read_html("https://cran.r-project.org/src/base/R-4/") |>
    html_elements("a") |> 
    html_text2() |> 
    str_extract("R-(.*)\\.tar\\.*", group=1) |> 
    na.omit() |> 
    package_version() |> 
    max()
```

As of `r Sys.Date()`, the most recent version of `R` is **`r latest_R`**. 
Using the most current version of `R` will reduce the likelihood of issues 
later in the course, so please upgrade even if you already have `R` installed.
...
Next, install [Quarto](https://quarto.org/docs/get-started/) for your platform
by clicking the link [here]([Quarto](https://quarto.org/docs/get-started/). We
won't actually use Quarto this week, but we will use it throughout the course, 
starting with [Mini-Project #00](../mini/mini00.html), so it's good to 
install now in case you have any questions. 


## Getting Started with `RStudio`

Go ahead and open `RStudio`. In the top right corner of your RStudio window, 
you should see a little icon that looks something like: 

![](../images/rstudio_project_logo.png)

Click next to that and select `New Project`. This will set up a _project_, 
which is essentially a folder in which _all_ work for this course will be done. 
Select "New Directory" as the project type, then "New Project", and finally name
your project `{{< var course.repo >}}`. Make sure to match the capitalization 
and the punctuation _exactly_. 

RStudio will then open your project and display `{{< var course.repo >}}` in
the top right corner next to the project icon. You should _always_ be working
in this project for this course. If you ever notice that you are outside
of the project, click and select `{{< var course.repo >}}` to re-open the 
project.
...
We will use the contributed `styler` package to format code in this course.[^air] 
Run the following command to automatically download and install the `styler` 
package: 

[^air]: The `air` formatter is a more modern replacement for `styler`, but it
can be a bit trickier to set up. If you'd like to use it in this class, 
follow the instructions [here](https://posit-dev.github.io/air/editor-rstudio.html).
...
[Mini-Project #00](../mini/mini00.html).
...
`GitHub` in more detail starting in [Mini-Project #00](../mini/mini00.html).
...
ambitious, go ahead and get started on [Mini-Project #00](../mini/mini00.html).
