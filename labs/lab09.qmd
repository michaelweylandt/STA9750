---
lab_num: "`r lab <- 9; lab`"
lab_title: "Reading and Manipulating HTML in `R`"
engine: knitr
---
...
5. Use the `unnest()` and `pivot_wider()` functions to convert this data into a 
...
3) Use the `html_table()` function to convert this table into a data frame. 
...
#| eval: false
library(rvest)
read_html("https://michael-weylandt.com/STA9750/labs/lab09.html") |>
...
CUNYs <- read_html("https://en.wikipedia.org/wiki/List_of_City_University_of_New_York_institutions") |> 
    html_element("tbody") |>
    html_elements("tr td:nth-child(2)") |> # Second column of each row
    html_elements("a") 
...
4) Modify the links extracted from the Wikipedia table to form full URLs.
   ...
   Proper URL handling is a bit subtle - it's a common avenue for 
   security holes - but a simple `paste0()` will suffice here. 
...
5) Apply your HTML parsing function from Step 1 to each CUNY page
...
6) Use your results to build a CUNY-wide map
...
If we want to add links to each page, we can do that as well, but we have to
handle rendering the appropriate HTML element properly. Here we'll write
and then process some markdown

```{r}
CUNYs <- CUNYs |> mutate(md = glue::glue("[{name}]({link})"), 
                         html = map_chr(md, commonmark::markdown_html), 
                         html2 = map(html, htmltools::HTML)) |> 
   unnest(html2)
with(CUNYs, 
     leaflet() |> 
        addTiles() |> 
        addMarkers(lng=longitude, 
                   lat=latitude, popup=html2))
```

Though this is a bit finicky because both `commonmark::markdown_html` and
`htmltools::HTML` try to combine their inputs by default, so we have to use
`map()` functions instead of simple vectorization.
...
**Save your code for next week.** Next week we will cover string processing
...
by processing the character column `Ingredient` and splitting it into separate
parts.
...
process_recipe <- function(recipe){
    # Question for reflection - why can't I vectorize this? 
    # Hint: It has to do with the fact different recipes have different numbers
    # of ingredients
