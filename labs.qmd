...
introduced in that week's [pre-assignment](./preassignments.qmd) are used in 
practice. For three weeks, student presentations will take the majority of
the class period and _optional_ (but recommended!) enrichment topics will be 
covered in the remaining class time.
...
lab_line <- function(N){
    if(str_detect(get_class_topic(N), "Lecture")){
        ...
        lab_n <- str_extract(topic, "Lecture #(\\d+)", group=1) |>
                as.integer()
        
        lab_file <- glue("./labs/lab{sprintf('%02d', lab_n)}.qmd")
        
        NN <- sprintf('%02d', lab_n)
        
        glue("\n    - [Lab #{NN}]({lab_file}): {get_lab_title(lab_n)}",
             .trim=FALSE)
    } else {
        ""
    }
