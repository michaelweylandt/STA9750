...
- `r PIAZZA_ENROLLED` students registered in Piazza
- `r TOTAL_STUDENTS - PIAZZA_ENROLLED` still need to register
...
- Still waiting on CUNY HR before TA office hourse begin
...
Most important for users of function are the first few lines (`args`)
...
- `x`: the input
- `base`: _optional_ second argument with a default $e$ (natural log)
...
Often, we want to save a set of related numbers as a single 'thing'
...
This is an _assignment_ operator. The vector formed by `1:5` is labeled `x`

Calling `x` later gives us this vector
...
We won't usually hand-write vectors like this: data comes in vectors
...
- [Pre-Assignment #0`r next_session`](../preassigns/pa04.html) due
