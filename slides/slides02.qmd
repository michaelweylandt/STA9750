...
#| include: false
TOTAL_STUDENTS <- 87
SYLLABUS_QUIZ_SUBMITTED <- 52
DISCUSSION_ENROLLED <- 46
```
...
You must submit the Brightspace Syllabus Quiz before you can submit any
graded assignments. 

- As of my last check, `r TOTAL_STUDENTS - SYLLABUS_QUIZ_SUBMITTED` students 
  still need to take the quiz(!)

## Discussion Board 

- `r DISCUSSION_ENROLLED` sign-ups on the Discussion Board: 
  - `r TOTAL_STUDENTS - DISCUSSION_ENROLLED` still need to sign up
...
- Due (just after) `r MP0_DATE |> strftime("%A, %B %d")`
- Possible tech issues, so start early

- If you've submitted on GitHub but haven't received a 'all checks passed'
  confirmation message, I still need your GitHub info via personal message
  ("Assignment") on the discussion board

## Optional DataCamp

_Optional_ DataCamp course: 
...
Aside, 'fixed-width font' (like the above) is used to emphasize that 
text should be read _as code_. *Cf.* Python where indentation (and hence 
horizontal spacing) matters and needs to be standardized
...
- WYSIWYM: **Code**, Chat formatting (Reddit, discord, slack), Word/Google Docs (power users)
- WYSIWYG: Image editing, Word/Google Docs (typical users), "No-Code" engines (Squarespace, Mailchimp)
...
For _results_ tables, we will use the [`gt` `R` package](https://gt.rstudio.com/)
starting in [MP#01](../mini/mini01.qmd)
...
Let's go practice using `Quarto` in [this week's lab](`r url_full`)
...
![](../images/git_pane_basic.png){fig-align="center" width="30%"}

:::

::: {.column width="50%"}

- Blue `M` are files that have been **M**odified 
...
![](../images/git_pane_added.png){fig-align="center" width="40%"}
...
**Warning**: Do not make any edits using the GitHub website. 

- This will cause you significant pain. 
- All changes should be made from one computer and pushed upwards only.
...
*LLMs can do this semi-automatically for you, but advice about what to search
still holds*
...
You should now start thinking about [Mini-Project #00](../mini/mini00.qmd)
...
- Register on course discussion board (Teams)
