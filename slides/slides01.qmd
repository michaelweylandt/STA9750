...
class_date:
  "`r library(tidyverse);
    ...
    filter(Course.Element == 'Class Session',
           Item.Number == session) |>
    pull(Date); 
    class_date |> format('%Y-%m-%d (%a)') |> paste(collapse=' and ')`"
format: revealjs
---
...
```{=html}
<iframe src="https://michael-weylandt.com/STA9750/" width="800" height="300" style="border: none;"></iframe>
```
...
::: {.incremental}
  ...
  - `r class_days` on Zoom (here!) at 6:05pm
...
Restricted to `@stu-mail.baruch.cuny.edu` emails - send a private message via the
discussion board if you have a different CUNY email and I'll special-case you
...
Extra credit for contribution to course materials or 'above and beyond' on 
...
- Brightspace configuration is fragile, so send a message via the 
  Discussion Board if issues
