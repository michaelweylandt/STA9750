...
1. When you are writing `quarto` documents in `RStudio`, 
...
2. `git` supports a huge variety of programming workflow
   ...
   3) You `git push` at the end of every work session. This
   ...
   4) You work in a "regular" file system. Synchronization
   ...
   5) You avoid anti-virus issues. If you are on a Windows
   ...
   6) You are careful about what files you `add` in `git`.
...
3. Use the file names and directory structures specified in
...
The following will automatically run the `lintr` package on a submitted `qmd`
document. 
