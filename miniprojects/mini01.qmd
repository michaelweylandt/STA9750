...
Create a new table from `USAGE` that has annual summary `UPT` and `VRM` for 2022. This will require use of the `group_by`, `summarize`, and `filter` functions. You will also want to use the `year` function, to extract a year from the `month` column.
