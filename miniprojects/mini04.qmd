...
This does not need be a _long_ bit of code when it is all done. My code to do 
all of these steps and to full clean and parse this data totals only 17 lines,
generously formatted, after a bit of trial-and-error to see which elements of
the HTTP request are not actually essential. 
...
3. Write a function to extract the desired data from the _2024_ table. (It will
   ...
   twelve-row data frame structured with the five columns as above. 
...
This should not be a _long_ bit of code when it is all done. My code to do all 
of these steps totals only 18 lines, generously formatted. 
...
- Applying computationally intensive techniques in your fact check. (You need to
  perform a theory-based parametric test, so the `infer` usage here would be a
  second statistical test of the same underlying claim.)
