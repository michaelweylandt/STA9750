...
2. The retirement benefit is calculated based on the _Final Average Salary_ of
   ...
   If $N$ is the number of years served, the retirement benefit is: 
...
with the following asset allocation:[^4]
...
the employee's ORP account. These returns are calculated as a percentage of
the employee's annual salary. Specifically, the employee contributes at the
same rate as the TRS: 
...
[^4] This is simplified from the actual rates used by Fidelity, but it's close
to accurate. 
...
::: {.callout-important title="Don't R Packages To Access Data Sources"}
...
relevant data in your training set. I'd recommend using around 20-30 years of 
...
::: {.callout-tip title="Task 7: Fixed-Rate Analysis"}
...
::: {.callout-tip title="Task 8: Monte Carlo Analysis"}
...
Finally, write up your findings from Task 8 in the form of a "data-driven
