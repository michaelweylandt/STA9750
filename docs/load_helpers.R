...
mp_pf_perform <- function(N, github = gitcreds::gitcreds_get()$username, secret){
    ...
    assigned_issues_decoded <- assigned_issues_encoded
    ...
    if(any(assigned_issues_decoded %not.in% pf_file$issue_status$number)){
    ...
    for(assignment_ix in seq_along(assigned_issues_decoded)){
        ...
        if(issue_dfr$ok){
            ...
            cat("- ❌ Submission failed automated checks with the following message: {issue_dfr$issue_status}\n")
        }
