...
DATES <- readr::read_csv("key_dates.csv") |>
    rename(element=`Course Element`,
           item=`Item Number`) |>
    mutate(Date = strftime(Date, "%B %d, %Y (%A)"), 
           dt = case_when(is.na(Time) ~ as.character(Date),
...
By
`{r} DATES |> filter(str_detect(Details, "Team Roster Submission")) |> pull(dt) |> unique()`,
submit a list of group members by posting (and tagging) all team members in a 
post in the relevant Teams channel. After the original list is posted other 
group members must reply acknowledging their intention to work with this group.
Once all listed team members have confirmed their participation in the team, 
the instructor will register the team in Brightspace, making it possible to 
access and submit to group assignments. 
...
the deadline; that is, teams where **all** team members have replied to 
the team formation post thread by the deadline. Teams registered after the
deadline will not receive full credit. 

:::: {multiple_sections="true"}

::: {.callout-important title="Team Days"}

When registering your team with the instructor, please indicate which section
(Tuesday or Thursday) each teammate is enrolled in. If you have a mix of 
students from different sections, please also indicate if your team would like
to do presentations on Tuesdays or Thursdays. This choice cannot be changed 
after the fact. 

It is not required for all team members to present for the first two rounds
of presentation, but it is required that all students are present for
final presentations, so please plan accordingly. *In particular, confirm that
any teammate who might need to skip another class will be able to do so.* If a
teammate is not available for final presentations, a significant penalty will be
applied to your final presentation grade. Because of this, I strongly recommend
trying to form a team comprised of students from your section.
...
it through Brightspace by `r DATES |> filter(str_detect(Details, "Contract")) |> pull(dt) |> paste(collapse = " or ")`:
...
On
`{r} DATES |> filter(str_detect(Details, "Project Proposal Presentation")) |> pull(Date) |> paste(collapse=" and ")`,
...
On
`r DATES |> filter(str_detect(Details, "Mid-Semester Check-Ins")) |> pull(Date) |> paste(collapse = " and ")`,
...
::: {.list-table width="1,1,1,1,1,1" aligns="c,l,l,l,l,l" header-rows=1 .hover #rubric-check-in}
  ...
  * - **Engagement with Relevant Literature**
    ...
    - (9-10/10) Excellent engagement with prior literature: team uses review of 
      existing work to tailor the project to fill a "gap" in the literature
  * - **Timing of Presentation** 
...
materially and holding to the team contract. Peer feedback must be submitted on
Brightspace by 
`{r} DATES |> filter(str_detect(Details, "Mid-Semester Teammate Peer Evaluations")) |> pull(dt) |> max()`.
Details of what to include in the peer feedback will be discussed in class.

Peer feedback will be collected using the `project_peer_evals()` function
on the ["Tips" page](tips.qmd#project_peer_evals) and submitted on Brightspace.
To run this script, simply run the following two commands at your `R` console:
...
On
`{r} DATES |> filter(str_detect(Details, "Final Presentations"), !str_detect(Details, "Slides")) |> pull(Date) |> paste(collapse=" and ")`,
...
*  - Integration of SQ Findings to Answer OQ (20)
   ...
   - SQ findings are qualitatively combined to give a partial answer to
     OQ. Limited effort is made to combine quantitative results of SQs in
     answering OQ. (9-12)
   ...
   - SQ findings are integrated in a sophisticated fashion, with significant
     ...
     quantitative story. OQ answer is clearly supported and takes full advantage
     of all SQs. (17-20)
*  - Engagement with Prior Literature (10)
   ...
   - Report discusses prior literature, but does not provide clear discussion
     of prior work and how it relates to present project. Key prior work
...
*  - Data Cleaning, Preparation, and Quality Control (10)
   ...
   - Significant data cleaning and preparation, requiring significant
     pre-processing and/or feature engineering. Data quality is assessed for
...
file. Requests must be made in writing and approved at least *one week* before 
the final individual report is due. 
...
i) "Synthetic Control": If the causal effect is some perturbation away from a 
...
`{r} DATES |> filter(str_detect(Details, "Final Project Teammate Peer Evaluations")) |> pull(dt)`).
