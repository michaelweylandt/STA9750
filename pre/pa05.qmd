...
::: {.callout-note title="`tribble`"}

In the following examples, I use the `tribble` function to quickly create a
data frame. The specifics of `tribble` are beyond the scope of this course, but
it essentially creates the data frame you would expect from looking at the code.
...
with some students who fail to submit certain assignments. 
...
- DataCamp: "Joining Data with `dplyr`" and "Reshaping Data with `tidyr`"
- Textbook: [*R4DS* §5](https://r4ds.hadley.nz/data-tidy.html) 
  and [*R4DS* §19](https://r4ds.hadley.nz/joins.html)
