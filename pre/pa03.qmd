...
You will see the `R` output sometimes has a `[1]` before it. Don't
worry about that until you get to the section on vectorized semantics below. 

Blocks that aren't listed as "Exercise" are interactive 
snippets. Feel free to adjust the code to check your
understanding and seeing what happens.
...
So above, `cos(pi)` implements the math $\cos(\pi)$. Here, 
we are calling the `cos` function with one argument, the scalar
value `pi`.
...
Here, our output is a _vector_ of 10 elements. `R` still tells us where the
vector starts (at the first element) but nothing else. Change this code to
create the first 100 elements and read `R`'s output. Do you understand the
output?
...
In this case, it is impossible to add the number 3 to a letter, so `R` 
throws an error. 
...
If you haven't done so already, make sure you complete the Syllabus Quiz or 
you won't be able to submit the quiz associated with this Pre-Assignment.

::: {.callout-note title="Additional Resources"}

- DataCamp: 
- Textbook: [*R4DS* §13.4-13.6](https://r4ds.hadley.nz/numbers.html#numeric-transformations)
