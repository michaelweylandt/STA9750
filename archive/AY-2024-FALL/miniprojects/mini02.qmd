...
mp <-  DATES |> filter(element == "Mini-Projects", 
                       item == mp_id)
mp_file <- list(qmd=glue("`mp{mp_id}.qmd`"), 
                html=glue("`mp{mp_id}.html`"),
                html_long=glue("`docs/mp{mp_id}.html`"))
```
...
> `https://<GITHUB_ID>.github.io/{{< var course.repo >}}/mp02.html`
...
3. There is one TV Episode in this data set with a perfect 10/10 rating    and at least 200,000 IMDb ratings. What is it? What series does it
   belong to?
