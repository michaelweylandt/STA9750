...
[applerankings.com](https://applerankings.com) is on the task. For this warm-up
exercise, we are going to extract his rankings of apples from 
<https://applerankings.com/pick-an-apple/> into a
four-column table of the following format: 
...
library(stringr)
todo <- "Yesterday, I needed to buy four cups of flour, a piece of Parmesan cheese, two gallons of ice cream, and a six-pack of bottled (non-alcoholic) beers."
str_extract_all(todo, " [:alnum:]+s[., ]", simplify=TRUE)
```
...
ending with an `s`. If you want to be more precise about this, look into 
functions like `pluralize::is_plural()`, but note that even that library 
can be tripped up by verbs ending with an `s`, *e.g.*, "goes".
...
library(stringr)
shopping <- "Today, I need to purchase 3 apples, 5 limes, and 2 lemons."
sum(as.numeric(str_extract_all(shopping, "[:digit:]+", simplify=TRUE)))
```
:::
...
RECIPES_LONG <- RECIPES_LONG |>
    mutate(unit = str_extract(ingredient, 
                              # Remove unit with optional plurals
                              " (oz|dash(es)?|drops?|t|chunks?|leaves|cm) ", 
                              group=1),
           # Remove trailing s or es (dashes => dash)
           unit = str_remove(unit, "e?s$"))
```
...
standardize_number <- function(x){
    library(stringr)
    x |> str_replace("([:digit:]+)½", "\\1.5") |> 
         str_replace("½", "0.5") |>
         str_replace("1/2", "0.5") |>
         str_replace("([:digit:]+)¾", "\\1.75") |> 
         str_replace("¾", "0.75") |>
         str_replace("3/4", "0.75") |>
         str_replace("([:digit:]+)¼", "\\1.25") |> 
         str_replace("¼", "0.25") |>
...
5.  You should wind up with a table that looks something like
    ...
    Combine the ingredient and unit columns so that `Meletti` and ` oz ` in 
...
RECIPES_LONG |>
    ...
    mutate(title  = str_trim(title), 
           ...
           unit = str_remove(unit, "e?s$"), 
           ingredient = str_remove_s(ingredient, unit), 
           ingredient = str_remove_s(ingredient, amount),
           # Remove any extra S / ES at the beginning from the unit stuff
           ingredient = str_remove_s(ingredient, "^ *e?s "),
           ingredient = str_trim(ingredient)) |>
...
There is more that could be done here, *e.g.*, standardizing ounces and 
teaspoons, but it's a pretty solid start.
