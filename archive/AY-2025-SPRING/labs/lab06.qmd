...
[^1]: We will not use `CVXR` in this course, but it is an incredibly useful tool
in the field of mathematical optimization, the branch of math that deals with
computationally finding the maxima or minima of functions when the simple "take 
the derivative and set to zero" approach is not workable. Optimization underlies
a huge swath of modern life: everything from fitting statistical models 
(_maximum_ likelihood) to route planning (_shortest_ path) to dynamic pricing
(revenue _maximizing_). To know it is to love it - we'll cover it a bit more in
STA 9890!
