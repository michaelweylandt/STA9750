...
In [Mini-Project #03](./mini/mini03.qmd), students will use US Census Bureau
estimates of state population and inter-state migration rates to forecast
state populations in the year 2030. Using these estimates, students will 
determine the number of congressional seats apportioned to each state. Finally,
using metro-to-metro migration flows, students will develop a targeted 
advertising strategy which aims to get more people to move to their state by
2030. 
...
In [Mini-Project #04](./mini/mini04.qmd), students will apply web-scraping 
techniques to acquire Olympic medal tables. Using this data, students will
develop a forecast of Team USA's performance at the upcoming LA 2028 Summer
Olympics. 
...
1) As a suitable HTML page hosted on the student's course repository. 
   ...
   setting this up.)
2) As a PDF on the course Brightspace page. Note that submission on Brightspace
   is required to access the associated Peer Feedback assignment. If you do
   not submit via Brightspace, you will not be able to see the Peer Feedback
   assignment (or get automated reminders) without manual intervention by the
   course staff.
...
- If the work is submitted on Brightspace by the deadline, but not on GitHub,
  the instructor will apply a 8-point penalty (10% deduction). Additionally,
  work not submitted on GitHub may not be eligible for peer review and
  may instead by evaluated by the course staff. (Note that, historically,
...
mp_submission_verify(3, "michaelweylandt")
  ...
  **Submissions that do not pass these automated checks will have a 5 point
  penalty applied.** 
  
- If the work is submitted on GitHub, but not on Brightspace, the instructor
  will assign a 8 point (10%) penalty. Note that this will be applied by
...
Note, however, I **will not** be necessarily be available to respond to student
inquiries after the original (Friday) deadline. You are *strongly* encouraged 
to ask all questions and resolve all technology issues before the Friday
deadline. 

**NB**: If you submit during the grace period, but after the official Friday
deadline, Brightspace will show a 0 automatically. This is just a temporary
grade and a real grade (unpenalized) will be replaced when I upload grades for
the whole class.[^bszero]

:::

[^bszero]: As far as I understand Brightspace, I have to set a 'non-submission'
rule that applies to all work submitted after the deadline. The choices are:
i) ignore the missing assignement entirely (and drop it from the grading); or
ii) assign an automatic zero. There seems to be no support for grace periods or
penalties - if you have seen a better set up in another course, please let me
know and I can look into it more deeply. (I don't want to get rid of the due 
date entirely as that would prevent students from getting notifications of
upcoming deadlines.)

Difficulty with the technologies used (Brightspace, `quarto`, GitHub, *etc.*) 
is not a recognized excuse for late submission.
...
**Use of `code-fold`ing is *not required*, but it is *strongly recommended*.**
...
Another useful option that you can set in the header of your document is
`code-summary`: adding something like 
...
will fold (hide) all code by default, while providing a button near the top of
the page to show everything. This provides a particularly nice experience for
anyone reading or evaluating your submission. 
...
it as it would make my document less clear. 

For this course, where reading the code is an important part of evaluating
each assignment, you *should generally not* use this option, but there may be
rare times it is useful. When in doubt, you should default to
include your code in your `qmd` document and using code-folding instead. 
...
but I'd say `code-fold`, `code-summary`, `echo` are the main ones in this class. 
...
The instructor will post structured files giving the peer feedback assignments
in the [submission repository](https://github.com/{{< var course.owner >}}/{{< var course.repo >}}) after the grace period ends. Once the assignment file has been posted for
a mini-project, an instructor-provided script will be used to decrypt the 
assignment file and to walk through the structured peer feedback process. After
finishing the peer feedback, a `bspf` file will be created. Upload this file
to Brightspace in order to submit your peer feedback. 

The instructor script can be accessed through the [helper scripts](tips.qmd#mp_pf_perform)
as follows: 
...
Note that this script is still new and may have some minor bugs. If you encounter
any trouble please contact the instructor for support ASAP. 
...
those where no submission can be found. Please use the feedback function
...
::: {.list-table width="1,2,3" aligns="c,l,l" header-rows=1 .hover #mp_meta_rubric}
...
Students who do not submit their mini-projects by the end of the grace period
will not be included in the peer feedback cycle unless special arrangements
are made persuant to the course [*Late Work Policy*](./syllabus.qmd#late-work-policy).
This is intended to ensure that only students who completed the assignment are
providing peer feedback and to minimize the time wasted by students who are asked
to provide feedback on non-existence submissions. As such, students who fail
to submit a mini-project on time without an approved excuse will receive a 0
on both portions of the assignment (initial submission and peer feedback.)

**Brightspace is configured so that you will only be able to submit Peer Feedback
after submitting the original assignment. If you do not have access to the Peer
Feedback assignment but have been asked to provide peer feedback, please contact
the course staff ASAP and a manual override can be put in place.**
