...
4.  Data scientists and business analysts are recorded under NAICS code `5182`.
    ...
    will need to create a new column to join on. In particular, the BLS will 
    record a code a "C1234" corresponding to a Census identifier of "12340"
    for the same region. To match these up, it is easiest to create a new column
    in the BLS tables which transforms values like "C1234" into "12340"-formatted
    values. 
    ...
    ```{r}
    #| eval: false
    _BLS TABLE_ <- _BLS TABLE_ |> mutate(_NEW COLUMN_ = code_changing(_BLS CBSA ID_))
    ...
    names surrounded by underscores to adapt this for your own use. You will
    also need to replace `code_changing` with an actual set of function calls. 
    
    There are several ways to do this, but the most straightforward is probably
    a combination of: 
    ...
    To determine the right order to use these, define a variable `x <- "C1234"`
    and figure out the sequence of function calls that satisfies: 
    
    ```{r}
    #| eval: no
    func1(func2(func3(x, argument?), argument?), argument?) == 12340
    ```
    
    Here, substitute `func1`, `func2`, `func3`, with some combination of the
    commands noted above and supply additional arguments where needed.
