...
In [Mini-Project #02](./miniprojects/mini02.html), students will explore data
from the US Census Bureau's Annual Community Survey and the Bureau of Labor
Statistics' Quarterly Census of Employment and Wages to identify cities with 
affordable and rapidly growing housing stock. In this project, students will
combine data from different - and not entirely compatible - government sources
and practice turning a qualitative concept like affordability into a measurable
statistical index. Students also demonstrate basic statistical visualizations
like scatterplots and line charts. Students will deliver their results in the
form of a "policy brief" that can be used to lobby congressional representatives
to advocate pro-housing policies at the federal level.
...
In [Mini-Project #03](./miniprojects/mini03.html), students will explore data
from the NYC Parks Tree Services. Students will create maps of the trees of
NYC  and use spatial joins to align the GPS coordinates of trees with NYC's
51 City Council districts. To acquire this data, students will also practice
responsibly downloading large data sets from an official government resource.
Students will deliver the findings in the form of a proposed new project for 
NYC Parks, aiming to maintain and benefit NYC's marvelous trees.
...
In [Mini-Project #04](./miniprojects/mini04.html), students will analyze the 
Bureau of Labor Statistics' Current Employment Statistics report to see whether
politicians' claims about CES revisions are actually true. To complete this
analysis, students will use the `httr2` package to directly access official
BLS CES records, parse the data into `R` using the `rvest` package, join
together records from different pages using `dplyr`, explore the data
with `ggplot2`, and finally apply computational approaches to statistical 
inference. 
...
verify that you have submitted a comment with the correct formatting.
